 ...
 */
  	...
  	public function creditsRecord()
    {
	    ...
	    switch ($type) {
		    case 0:
		    	$table = "AllRecord";
		    	$where["user_login"] = $uid;
            	$where["wallet"] = "credits";
		    	break;
		    case 1:
		    	$table = "UserOpCredits";
            	$where["fromuid|touid"] = $uid;
		    	break;
		    case 2:
		    	$table = "UserRoom";
		    	$where["uid"] = $uid;
	    ...
	    $data = M($table)->where($where)->page($page, $pageSize)->select();
      	$totalSize = M($table)->where($where)->select();
      	$users = array();
      	$tmp = array();
      	if($type == 0) {
          	$qunData = M("qun")->where(["uid" => $uid])->page($page, $pageSize)->select();
          	$idx = 0;
          	foreach($qunData as $item) {
            	$qunZhu = M("user")->where(["id" => $item["open"]])->find();
              	$tmp[$idx]["userimg"] = $qunZhu["img"];
              	$tmp[$idx]["number"] = ($item["credits"] > 0 ? "+" : "-") . abs($item["credits"]);
              	$tmp[$idx]["create_time"] = $item["createtime"];
              	$idx++;
            }
          	foreach($data as $item) {
            	$tmp[$idx]["userimg"] = "/app/img/member_union.png";
              	$tmp[$idx]["number"] = $item["number"];
              	$tmp[$idx]["create_time"] = strtotime($item["create_time"]);
              	$idx++;
            }
          	$data = $tmp;
        } else if($type == 1) {
      		foreach($data as $item) {
              	$users[$item["id"]] = M("user")->where(array("id" => $item["fromuid"]))->find();
        	}
        } else if($type == 2) {
        	foreach($data as &$item) {
            	$item["userimg"] = "/app/img/member_union.png";
            }
        }
      	$this->assign("self", $this->user);
	    $this->assign("users", $users);
	    $this->assign("type", $type);
	    $this->assign("recordData", $data);
      	ob_start();
	    $this->display();
      	$html = ob_get_contents();
      	ob_end_clean();
      	exit(json_encode(array("page" => $page, "pageSize" => $pageSize, "curPageSize" => count($data), "totalSize" => count($totalSize), "html" => $html)));
    }
