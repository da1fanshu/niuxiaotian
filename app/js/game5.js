function adduser(data){
    ...
    var html='<div class="member member'+indexuser+'" id="user'+data.user.id+'">'
     ...
     html+='<img src="/app/img/dougongniu/readyButton.png" class="unready" '+actxx+'/>' 
...
function overroom(data){
   ...
   overzt=0;
    ...
    img.onload = function(){
        ...
        ctx.fillText("房间号:"+sj.id, 178,285);
        ctx.fillText(time1, 361, 285);
        ctx.fillText(sj.zjs+"局", 580, 285);

        ctx.font = "30px bolder songti";
        ctx.fillStyle = "black";
        var writeContent = '牛小天'+'房主 ：'+fangzhu.nickname;
        var writeLeft = (c.width-ctx.measureText(writeContent).width)/2;
        ctx.fillText(writeContent,writeLeft,350);

        for(var i=0;i<sj.user.length;i++){
            if(i>1){
                ctx.fillStyle = "#140f2d";
                ctx.fillRect(142,610-(0-(i-2)*105),519,100);
            }
            var user=sj['user'][i];
            ctx.font = "30px bolder songti";
            ctx.fillStyle = "#d8cb66";
            ctx.fillText(decode64(user.nickname).substring(0, 15), 225, 460+i*105);
            if(user.dqjf>0){
                user.dqjf='+'+user.dqjf;
            }else{
                ctx.fillStyle = "#fff";
                ctx.fillText(decode64(user.nickname).substring(0, 15), 225, 460+i*105);
                ctx.fillText(user.dqjf, 580, 460-(0-i*105));
            }
            ctx.fillText(user.dqjf, 580, 460-(0-i*105));
        }
