...
function overroom(data){
    ...
    img.onload = function(){
        ...
        ctx.fillText("房间号:"+sj.id, 185,287);
        ctx.fillText(time1, 375, 287);
        ctx.fillText(sj.zjs+"局", 576, 287);
        ...
        for(var i=0;i<sj.user.length;i++){
            if(i>1){
                ...
                ctx.fillRect(141,574-(0-(i-2)*97),517,93);
            }
            var user=sj['user'][i];
            ctx.font = "33px bolder songti";
            ctx.fillStyle = "#d8cb66";
            //ctx.fillText(user.id, 195, 380+(i*75));
            ctx.fillText(decode64(user.nickname).substring(0,9), 219, 450+(i*94));
            if(user.dqjf>0){
                user.dqjf='+'+user.dqjf;
            }else{
                ctx.fillStyle = "#fff";
                ctx.fillText(decode64(user.nickname).substring(0,9), 219, 450+(i*94));
                ctx.fillText(user.dqjf, 560, 450+(i*94));
            }
            ctx.fillText( user.dqjf, 560, 450+(i*94));
        }
