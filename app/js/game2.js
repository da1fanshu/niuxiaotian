...
function overroom(data){
    ...
    img.onload = function(){
        ...
        ctx.fillText("房间号:"+sj.id, 190,270);
        ctx.fillText(time1, 375, 270);
        ctx.fillText(sj.zjs+"局", 566, 270);

        ctx.font = "28px bolder songti";
        ctx.fillStyle = "black";
        var writeContent = '牛小天'+'房主:'+fangzhu.nickname;
        var writeLeft = (c.width-ctx.measureText(writeContent).width)/2;
        ctx.fillText(writeContent,writeLeft,335);

        for(var i=0;i<sj.user.length;i++){
            if(i>1){
                ctx.fillStyle = "#19170b";
                ctx.fillRect(141,492-(0-(i-2)*77),518,73);
            }
            var user=sj['user'][i];
            ctx.font = "28px bolder songti";
            ctx.fillStyle = "#d8cb66";
            //ctx.fillText(user.id, 215, 390+(i*75));
            ctx.fillText(decode64(user.nickname).substring(0,11), 215, 397+(i*75));
            if(user.dqjf>0){
                user.dqjf='+'+user.dqjf;
            }else{
                ctx.fillStyle = "#fff";
                ctx.fillText(decode64(user.nickname).substring(0,11), 215, 397+(i*75));
                ctx.fillText(user.dqjf, 568, 397+(i*75));
            }
            ctx.fillText( user.dqjf, 568, 397+(i*75));
        }
