    ...
    img.onload = function(){
        ...
        for(var i=0;i<sj.user.length;i++){
            if(i>1){
                ...
                ctx.fillRect(145,510-(0-(i-2)*70),515,64);
            }
            var user=sj['user'][i];
            ctx.font = "31px bolder songti";
            ctx.fillStyle = "#d8cb66";
            //ctx.fillText(user.id, 195, 380+(i*75));
            ctx.fillText(decode64(user.nickname).substring(0,10), 219, 420+(i*69));
            if(user.dqjf>0){
                user.dqjf='+'+user.dqjf;
            }else{
                ctx.fillStyle = "#fff";
                ctx.fillText(decode64(user.nickname).substring(0,10), 219, 420+(i*69));
                ctx.fillText(user.dqjf, 585, 420+(i*69));
            }
            ctx.fillText( user.dqjf, 585, 420+(i*69));
        }
